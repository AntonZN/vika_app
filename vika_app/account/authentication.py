...
class BaseFirebaseAuthentication(authentication.BaseAuthentication):
    ...
    def authenticate(self, request):
        ...
        if settings.DEBUG:
            try:
                if "swagger-rest" in request.headers.get("Referer").split("/"):
        ...
        local_user = self.get_or_create_local_user(firebase_user=firebase_user)
    ...
    def get_or_create_local_user(self, firebase_user=None):
        raise NotImplementedError(
            "get_or_create_local_user() has not been implemented."
        )

    def create_local_firebase_user(self, local_user, firebase_user):
        raise NotImplementedError(
            "create_local_firebase_user() has not been implemented."
        )
...
class FirebaseAuthentication(BaseFirebaseAuthentication):
    ...
    def decode_token(self, firebase_token):
        ...
        """
        try:
            tokent = firebase_auth.verify_id_token(
                firebase_token, check_revoked=api_settings.FIREBASE_CHECK_JWT_REVOKED
            )
            return tokent
    ...
    def authenticate_token(self, decoded_token):
        ...
        try:
            ...
            raise exceptions.AuthenticationFailed(
                "Error retrieving the user, or the specified user ID does not " "exist"
            )
    ...
    def get_or_create_local_user(self, firebase_user=None):
        ...
        """
        if settings.DEBUG:
            user = User.objects.get(phone="+79646260040")
        ...
        try:
            ...
            new_user = User.objects.create(
                phone=phone, user_type=User.UserType.USER, last_login=timezone.now(),
            )
            return new_user
